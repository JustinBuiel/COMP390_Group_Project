...
def get_single_listing_product_name():
    """ When called, this function makes a GET request to a hard coded URL. The request is directed to an
    Amazon product page addressed by the following URL: 'https://www.amazon.com/dp/B08J5F3G18/'.
    The page is the dedicated product listing for an EVGA graphics card.
    The response content (HTML) is parsed and the product name is extracted and printed to a text file
    and to the terminal. """

    # make/open a text file to store the product name - open the file in 'write' mode ('w')
    data_out_file = open("output_data.txt", "w")

    # "B08J5F3G18" at the end of this URL is the ASIN (Amazon Standard Identification Number) for a single product
    product_url = 'https://www.amazon.com/dp/B08J5F3G18/'
    # issue GET request - be sure to include the header information - assign it to the 'get()' function's 'headers'
    # parameter
    web_response = requests.get(product_url, headers=HEADERS_FOR_GET_REQ)

    # Construct a Beautiful Soup (BS) object. This object will help parse the HTML code text.
    # The BS object has methods and properties that are useful when parsing HTML.
    # (('web_response.content' contains all the HTML code returned by the GET request's target URL))
    # Use the standard Python 'html.parser' class as a base for the BS parser. The 'html.parser' class is defined
    # in the standard Python library (Lib/html/parser.py).
    b_soup_format = BeautifulSoup(web_response.content, "html.parser")

    product_title_string = ''
    try:
        # The BS .find() method returns a single block of HTMl that has the provided <tag> and contains the
        # provided attribute value(s). The first instance is returned (the entire HTML code is not scanned).
        # https://www.crummy.com/software/BeautifulSoup/bs4/doc/#find

        # With some investigation into the HTML, it was discovered that the first <span> block with its 'id'
        # attribute set to "productTitle" contains the product title string.
        product_title = b_soup_format.find("span", attrs={"id": 'productTitle'})
        print(product_title)
        # 'product_title' is an instance of a BS-defined class (<class 'bs4.element.Tag'>)
        print(f'{type(product_title)}\n')
        # Get the '.string' property to extract the string portion of the 'element.Tag' class
        product_title_value = product_title.string
        print(product_title_value)
        # 'product_title_value' is an instance of a BS-defined class (<class 'bs4.element.NavigableString'>)
        print(f'{type(product_title_value)}\n')
        # use the standard Python Library string method '.strip()' to get rid of any leading or trailing whitespace.
        # The '.strip()' method returns a Python 'string' type (str). Having the product title as a Python 'str'
        # type is much more convenient.
        product_title_str = product_title_value.strip()
        print(product_title_str)
        # 'product_title_str' is a str type
        print(f'{type(product_title_str)}\n')

    # An AttributeError exception will be thrown if the BS '.find()' method fails to find a given HTML tag block with
    # the provided attribute values.
    except AttributeError:
        # In this case, an attribute error indicates the product page has no product title.
        # This is very unlikely considering that any product listing should AT LEAST have a product title.
        # If it happens that no HTML <span> black has its 'id' attribute set to "productTitle", inform the user that
        # there is no product title available. Again, this is very unlikely.
        product_title_string = "NA"

    # print the product title string to the text file
    data_out_file.write(f'{product_title_string}')
    data_out_file.close()
